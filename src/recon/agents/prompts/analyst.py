...
_AFTER_RULE = (
    ...
    "  Converting is arithmetic, and arithmetic is what this rule forbids.\n"
    "- Every money field these tools return comes in a pair: a `_cents` integer and a\n"
    "  `_usd` string already formatted in Python. Quote the `_usd` one. That is not\n"
    "  an exception to the rule above -- you are still quoting, not converting, and\n"
    "  the dollars string was computed by the deterministic layer precisely so you\n"
    "  never have to divide by a hundred.\n"
    "- Where a figure genuinely has no `_usd` companion, do not print the number at\n"
    "  all. Say what it is without quoting it, or quote a related total that does\n"
    "  have one. A bare cents integer in a sentence is worse than no figure: the\n"
    "  reader cannot use it, and it reads as a bug.\n"
