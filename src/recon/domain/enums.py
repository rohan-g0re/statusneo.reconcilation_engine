...
class KeyType(StrEnum):
    """Crosswalk key types, one per bridge in ``feed_formats.md`` §5.

    Canonical string forms, pipe-separated, so a key is one indexed lookup:

    ``NCPDP_TXN``            ``pharmacy_npi|rx_number|fill_number|date_of_service``
    ``NATURAL_340B``         ``pharmacy_npi|rx_number|ndc11|date_of_service``
    ``NATURAL_340B_MEDICAL`` ``provider_npi|ndc11|service_date`` (Decision 37)
    ``CLP01_PARSED``         ``rx_number|fill_number`` parsed out of CLP01
    ...
    NCPDP_TXN = "NCPDP_TXN"
    NATURAL_340B = "NATURAL_340B"
    NATURAL_340B_MEDICAL = "NATURAL_340B_MEDICAL"
    CLP01_PARSED = "CLP01_PARSED"
    CLM01 = "CLM01"
    CLP07 = "CLP07"
    TRN02 = "TRN02"
    ACH_TRACE = "ACH_TRACE"
