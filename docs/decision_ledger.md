...
### A.7 — Agent layer surface

| #   | Decision                                                                                                                | Notes                                                                                                                                                                                                                                                                                                                                                                                                 |
| --- | ------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A32 | ✅**The cross-episode to-do list is a read-only panel on the dashboard, not its own screen** | **Reverses `agent_layer_design.md` §8.6 as first written**, which specified the to-do list as its own screen — "different questions, different screens." User decision: shown "own screen at `/todos`" and chose "panel on dashboard" instead; shown a `work_item_closure` append-only table that would let items be checked off, and chose to stay read-only instead. Both alternatives declined, not merely unconsidered. Read-only is load-bearing, not laziness: `work_item` is append-only at the schema-trigger level, `UPDATE` and `DELETE` both abort (`src/recon/db/schema.sql:461-464`), so there is no done state for a checkbox to reach for. The panel filters client-side to `at_cursor <= cursor` and shows an explicit count of what that hides — to-dos recorded after the dashboard's replay cursor are counted on screen, never silently dropped, the same discipline every other element on that screen already follows. Needed no new table, no schema change and no new endpoint: it consumes `GET /api/agent/work-items`, which already existed. The per-episode to-do list on `/analyse/{episode_id}` is unchanged |
