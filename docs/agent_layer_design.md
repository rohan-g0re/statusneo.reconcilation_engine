...
*Added after implementation, because this section was written before the tool layer existed and never named the thing it all routes through.* **Every one of the nine tools reads through one function, `dossier.build_dossier()`** — `tools.py` imports it and no tool reaches the database any other way. That single entry point is what makes the read surface auditable: there is one place to check what an agent can see, rather than nine.
